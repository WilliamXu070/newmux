...
if [ "$(uname)" = Darwin ] && [ -d /Applications/Ghostty.app ]; then
	exec open -na Ghostty.app --args --config-file="$ROOT/ghostty/newmux.config"
	...
	echo "  ghostty --config-file=$ROOT/ghostty/newmux.config" >&2
	exit 1
fi

exec "$GHOSTTY" --config-file="$ROOT/ghostty/newmux.config"
